import { appRoutes, getRouteType, type RouteType } from "@/common/app-routes";
...
const getRedirectPathname = (path: string) => {
  try {
    return new URL(path, "https://local.app").pathname;
  } catch {
...
export const getSafeRedirectPath = (
  ...
  const fallback = options.fallback ?? appRoutes.postLogin.base;
  if (!value) {
    return fallback;
  }
