...
export default function AdminClaimDetailPage({
  ...
  const { data: claim, isLoading: claimLoading } = useQueryClaim(claimId);
  const { data: events = [] } = useQueryClaimEvents(claimId);
  const approveMutation = useMutApproveClaim();
  const rejectMutation = useMutRejectClaim();
