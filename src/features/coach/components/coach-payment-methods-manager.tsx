...
import {
  type CoachPaymentMethodFormData,
  coachPaymentMethodSchema,
} from "@/features/coach/schemas";

interface CoachPaymentMethodItem {
  id: string;
  type: PaymentMethodType;
...
interface CoachPaymentMethodsManagerProps {
  coachId: string;
  sectionId?: string;
  onMethodChanged?: () => void;
}

export function CoachPaymentMethodsManager({
  coachId,
  sectionId,
  onMethodChanged,
}: CoachPaymentMethodsManagerProps) {
  const paymentMethodsQuery = useQueryCoachPaymentMethods(coachId);
  const createPaymentMethod = useMutCreateCoachPaymentMethod(coachId);
  const updatePaymentMethod = useMutUpdateCoachPaymentMethod(coachId);
  const deletePaymentMethod = useMutDeleteCoachPaymentMethod(coachId);
  const setDefaultPaymentMethod = useMutSetDefaultCoachPaymentMethod(coachId);

  const [paymentMethodDialogOpen, setPaymentMethodDialogOpen] =
    React.useState(false);
  const [editingPaymentMethod, setEditingPaymentMethod] =
    React.useState<CoachPaymentMethodItem | null>(null);
  const [paymentMethodToDelete, setPaymentMethodToDelete] =
    React.useState<CoachPaymentMethodItem | null>(null);

  const defaultPaymentMethodValues: CoachPaymentMethodFormData = {
    type: "MOBILE_WALLET",
  ...
  const paymentForm = useForm<CoachPaymentMethodFormData>({
    resolver: zodResolver(coachPaymentMethodSchema),
    mode: "onChange",
  ...
  const handleEditPaymentMethod = (method: CoachPaymentMethodItem) => {
  ...
  const handleSavePaymentMethod = async (
    values: CoachPaymentMethodFormData,
  ) => {
    const payload = {
      type: values.type,
  ...
  const handleTogglePaymentMethod = async (method: CoachPaymentMethodItem) => {
    try {
      await updatePaymentMethod.mutateAsync({
  ...
  const handleSetDefault = async (method: CoachPaymentMethodItem) => {
    try {
      ...
      <Card id={sectionId}>
        <CardHeader className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="space-y-1">
            ...
            <CardDescription>
              Add mobile wallets and bank accounts for coach booking payments.
            </CardDescription>
          </div>
          <Button
            type="button"
        ...
        <CardContent className="space-y-4">
          {paymentMethodsQuery.isLoading ? (
            ...
            <div className="rounded-lg border border-dashed p-6 text-center text-sm text-muted-foreground">
              No payment methods yet. Add at least one to receive coach booking
              payments.
            </div>
          ) : (
        ...
        <DialogContent className="sm:max-w-[520px]">
          ...
          >
            <div className="grid gap-4 sm:grid-cols-2">
              <StandardFormSelect<CoachPaymentMethodFormData>
                name="type"
                label="Method Type"
            ...
            <StandardFormInput<CoachPaymentMethodFormData>
              name="accountName"
              label="Account Name"
            ...
            <StandardFormInput<CoachPaymentMethodFormData>
              ...
              required
            />
            <StandardFormField<CoachPaymentMethodFormData>
              name="instructions"
            ...
            <div className="flex flex-wrap items-center gap-6">
              <StandardFormSwitch<CoachPaymentMethodFormData>
                name="isDefault"
