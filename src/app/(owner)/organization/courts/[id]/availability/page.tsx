...
export default async function OwnerCourtAvailabilityAliasPage({
  ...
  const pathname =
    ...
    appRoutes.organization.courts.availability(id);
  ...
  if (!placeId) {
    redirect(appRoutes.organization.courts.base);
  }

  redirect(appRoutes.organization.places.courts.availability(placeId, id));
}
