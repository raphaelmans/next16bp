...
export default function CreatePlaceCourtSetupPage() {
  ...
  const { submit, isSubmitting } = useCourtForm({
    onSuccess: (result) => {
      ...
      router.push(
        `${appRoutes.owner.places.courts.setup(placeId, result.courtId)}?step=hours`,
      );
    },
  });
  ...
  return (
    ...
    >
      <div className="space-y-6">
        <PageHeader
          title={`Create Court · ${placeData.place.name}`}
