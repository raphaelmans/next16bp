...
import { GetOrgReservationsSchema } from "@/lib/modules/reservation/dtos";
...
type GetOrgReservationsMobileResponse = Awaited<
  ReturnType<IReservationOwnerService["getForOrganization"]>
>;
...
export async function GET(req: Request, context: { params: Params }) {
  ...
  try {
    ...
    const input = validate(GetOrgReservationsSchema, {
      organizationId,
      reservationId: getStringParam(query, "reservationId"),
      placeId: getStringParam(query, "placeId"),
      courtId: getStringParam(query, "courtId"),
    ...
    const result = await service.getForOrganization(session.userId, input);

    return NextResponse.json<ApiResponse<GetOrgReservationsMobileResponse>>(
      wrapResponse(result),
    );
  } catch (error) {
