...
import { ConvertWalkInBlockSchema } from "@/lib/modules/reservation/dtos";
...
export async function POST(req: Request, context: { params: Params }) {
  ...
  try {
    ...
    const input = validate(ConvertWalkInBlockSchema, {
      ...(raw as Record<string, unknown>),
      blockId,
    });

    const service = makeReservationOwnerService();
    const result = await service.convertWalkInBlockToGuest(
      session.userId,
      input,
    );
