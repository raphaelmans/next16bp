...
import { CancelCourtBlockSchema } from "@/lib/modules/court-block/dtos";
...
export async function POST(req: Request, context: { params: Params }) {
  ...
  try {
    ...
    const input = validate(CancelCourtBlockSchema, { blockId });
    const service = makeCourtBlockService();
    const result = await service.cancelBlock(session.userId, input);
