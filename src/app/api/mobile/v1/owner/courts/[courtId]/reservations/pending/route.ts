...
import { GetPendingForCourtSchema } from "@/lib/modules/reservation/dtos";
...
export async function GET(req: Request, context: { params: Params }) {
  ...
  try {
    ...
    const input = validate(GetPendingForCourtSchema, { courtId });
    const service = makeReservationOwnerService();
    const result = await service.getPendingForCourt(
      session.userId,
      input.courtId,
    );
