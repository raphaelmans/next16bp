...
import { ListSourcesSchema } from "@/lib/modules/bookings-import/dtos";
...
export async function GET(req: Request, context: { params: Params }) {
  ...
  try {
    ...
    const input = validate(ListSourcesSchema, { jobId });
    const service = makeBookingsImportService();
    const result = await service.listSources(session.userId, input.jobId);
