...
import { ListJobsSchema } from "@/lib/modules/bookings-import/dtos";
...
export async function GET(req: Request) {
  ...
  try {
    ...
    const input = validate(ListJobsSchema, {
      placeId: getStringParam(query, "placeId"),
    });

    const service = makeBookingsImportService();
    const result = await service.listJobs(session.userId, input.placeId);
