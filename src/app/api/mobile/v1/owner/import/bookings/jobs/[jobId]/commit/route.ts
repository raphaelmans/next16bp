...
import { CommitJobSchema } from "@/lib/modules/bookings-import/dtos";
...
export async function POST(req: Request, context: { params: Params }) {
  ...
  try {
    ...
    const input = validate(CommitJobSchema, { jobId });
    const service = makeBookingsImportService();
    const result = await service.commit(session.userId, input.jobId);
