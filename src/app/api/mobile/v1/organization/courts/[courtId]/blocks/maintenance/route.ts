...
import { CreateCourtBlockSchema } from "@/lib/modules/court-block/dtos";
...
export async function POST(req: Request, context: { params: Params }) {
  ...
  try {
    ...
    const input = validate(CreateCourtBlockSchema, {
      ...(raw as Record<string, unknown>),
      courtId,
    });

    const service = makeCourtBlockService();
    const result = await service.createMaintenance(session.userId, input);
