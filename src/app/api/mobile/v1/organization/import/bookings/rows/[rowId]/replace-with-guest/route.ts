...
import { ReplaceWithGuestSchema } from "@/lib/modules/bookings-import/dtos";
...
export async function POST(req: Request, context: { params: Params }) {
  ...
  try {
    ...
    const input = validate(ReplaceWithGuestSchema, {
      ...(raw as Record<string, unknown>),
      rowId,
    });

    const service = makeBookingsImportService();
    const result = await service.replaceWithGuest(session.userId, input);
