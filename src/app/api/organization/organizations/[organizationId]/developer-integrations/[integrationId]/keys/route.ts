...
export async function GET(req: Request, context: { params: Params }) {
  ...
  try {
    ...
    const input = validate(ListDeveloperApiKeysSchema, {
      organizationId,
      integrationId,
    });
    const service = makeDeveloperIntegrationService();
    const result = await service.listApiKeys(session.userId, input);
...
export async function POST(req: Request, context: { params: Params }) {
  ...
  try {
    ...
    const input = validate(CreateDeveloperApiKeySchema, {
    ...
    const service = makeDeveloperIntegrationService();
    const result = await service.createApiKey(session.userId, input);
