...
import { GET } from "@/app/api/mobile/v1/owner/chat/reservations/thread-metas/route";
...
describe("GET /api/mobile/v1/owner/chat/reservations/thread-metas", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it("reservationIds only -> calls service with object payload and defaults", async () => {
    ...
    const req = new Request(
      `https://example.com/api/mobile/v1/owner/chat/reservations/thread-metas?reservationIds=${RESERVATION_ID_1},${RESERVATION_ID_2}`,
    );
  ...
  it("reservationGroupIds only -> calls service with empty reservationIds", async () => {
    // Arrange
    const req = new Request(
      `https://example.com/api/mobile/v1/owner/chat/reservations/thread-metas?reservationGroupIds=${GROUP_ID_1}`,
    );
  ...
  it("mixed ids + includeArchived=true -> forwards all query inputs", async () => {
    // Arrange
    const req = new Request(
      `https://example.com/api/mobile/v1/owner/chat/reservations/thread-metas?reservationIds=${RESERVATION_ID_1}&reservationGroupIds=${GROUP_ID_1},${GROUP_ID_2}&includeArchived=true`,
    );
  ...
  it("no query params -> forwards empty ids and returns wrapped response", async () => {
    // Arrange
    const req = new Request(
      "https://example.com/api/mobile/v1/owner/chat/reservations/thread-metas",
    );
