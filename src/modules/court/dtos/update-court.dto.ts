...
export const UpdateReservableCourtDetailSchema = z.object({
  ...
  paymentHoldMinutes: z.number().int().min(1).max(1440).optional(),
  ownerReviewMinutes: z.number().int().min(1).max(1440).optional(),
  cancellationCutoffMinutes: z.number().int().min(0).max(1440).optional(),
