...
export type SitemapBuckets = Record<SitemapBucketName, MetadataRoute.Sitemap>;

const buildUrl = (path: string) => buildCanonicalUrl(path);

export async function getSitemapBuckets(): Promise<SitemapBuckets> {
  ...
  const editorialEntries: MetadataRoute.Sitemap = [
    {
      url: buildUrl(appRoutes.guides.base),
      changeFrequency: "weekly" as const,
      priority: 0.6,
    },
    ...GUIDE_ENTRIES.map((guide) => ({
      url: buildUrl(appRoutes.guides.detail(guide.slug)),
      lastModified: new Date(guide.updatedAt),
      changeFrequency: "monthly" as const,
      priority: guide.audience === "players" ? 0.7 : 0.55,
    })),
  ];
  ...
  return {
    ...
    editorial: editorialEntries,
    locations: locationEntries,
    venues: [...placeEntries, ...placeReviewEntries],
