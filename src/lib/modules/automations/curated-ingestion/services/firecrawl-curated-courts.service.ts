...
function parseArgs(): ScriptOptions {
  ...
  for (let i = 0; i < args.length; i += 1) {
    ...
    if (arg === "--trust-urls-file") {
      options.trustUrlsFile = true;
      continue;
    }
...
function selectCandidateUrls(
  ...
  const baseCandidates =
    options.urlsFilePath && options.trustUrlsFile
      ? urls
      ...
      : urls
          ...
          .filter((url) => {
            ...
            return (
              host === startHost || getRootDomain(host) === startRootDomain
            );
          })
...
export function runFirecrawlCuratedCourtsCli(cliArgs?: string[]) {
  return runCliWithOptionalArgs(cliArgs, main);
}

if (isDirectExecution(import.meta.url)) {
  runFirecrawlCuratedCourtsCli().catch((error) => {
    console.error(error);
    process.exit(1);
  });
}
